...
\begin{tikzpicture}[
  ...
  \node[aplearn, text width=13.1cm, anchor=north] (author) at (0,0)
    {\textbf{A language model writes the scene specification}\\[1.5pt]
     \apsub{the bodies, their initial positions and velocities, the static
     surfaces they interact with,\\ and a camera. A body on a ballistic segment
     carries an initial velocity and its colliders;\\ prescribed waypoints are
     reserved for motion the simulator would otherwise override, such as\\ a
     rising balloon. A reply missing a required field, or naming a control strength
     outside\\ the allowed range, is re-asked once}};

  \node[apdata, text width=6.8cm, anchor=south]
    (spec) at ($(author.north)+(0,0.40)$)
    {\textbf{Physics description, in text}\\[1.5pt]
     \apsub{the physical event the clip is\\ required to show}};
  ...
  \node[apdet, text width=13.1cm, anchor=north]
    ...
    {\textbf{The specification is simulated by the same engine the renderer runs}\\[1.5pt]
     \apsub{the rigid-body simulator integrates the scene, producing the arc, the
     contact and the settling.\\ The trajectory checked below is the one the control
     video will show}};

  % ---------------- validation on the realized trajectory (accented) -------
  \node[apgate, text width=13.1cm, anchor=north]
    (gate) at ($(sim.south)+(0,-0.34)$)
    {{\color{adobered}\textbf{Validation is applied to the simulated trajectory}}\\[1.5pt]
     \apsub{a ballistic segment must trace an arc that reaches its target, or
     cleanly misses it where the\\ corrected outcome requires, and come to rest on
     a surface or on another body; a rising\\ segment must gain height; no body may
     leave the frame; a segment declared ballistic must be\\ simulated}};
  ...
  \node[aplearn, text width=6.2cm, anchor=north east]
    ...
    {\textbf{Appearance is written separately}\\[1.5pt]
     \apsub{a second request, given no access to the\\ control, describes the scene
     as filmed;\\ replies naming the control's own\\ appearance are rewritten}};
  ...
  \node[apdet, text width=13.1cm, anchor=north]
    ...
    {\textbf{The camera is applied to the accepted specification}\\[1.5pt]
     \apsub{a panning camera becomes a slow uniform drift of the whole scene; an
     orbit or a push-in is left\\ to the generator; bodies serving as graphics are
     dropped here}};
  ...
  \node[apdet, text width=13.1cm, anchor=north]
    ...
    {\textbf{The accepted specification is rendered into the control video}\\[1.5pt]
     ...
     from the simulated\\ state over the clip's fixed frame budget, with a contact
     sheet saved alongside}};

  \node[apdet, text width=13.1cm, anchor=north]
    (planrow) at ($(render.south)+(0,-0.34)$)
    {\textbf{What is handed to the generator}\\[1.5pt]
     \apsub{the control video, the appearance description, the excluded terms, a
     fixed seed, and the\\ strength at which the control is applied, which is
     lowered when the control covers little\\ of the frame}};
  ...
  \node[aplearn, text width=13.1cm, anchor=north]
    ...
    {\textbf{The diffusion backbone follows the control video}\\[1.5pt]
     ...
     and environment from\\ the appearance description}};
